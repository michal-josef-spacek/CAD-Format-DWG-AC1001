...
sub _read {
    ...
    for (my $i = 0; $i < $n_blocks; $i++) {
        push @{$self->{blocks}}, CAD::Format::DWG::AC1001::Block->new($self->{_io}, $self, $self->{_root});
    }
    $self->{layers} = ();
    my $n_layers = $self->header()->table_layer()->items();
    for (my $i = 0; $i < $n_layers; $i++) {
        push @{$self->{layers}}, CAD::Format::DWG::AC1001::Layer->new($self->{_io}, $self, $self->{_root});
    }
    $self->{styles} = ();
    my $n_styles = $self->header()->table_style()->items();
    for (my $i = 0; $i < $n_styles; $i++) {
        push @{$self->{styles}}, CAD::Format::DWG::AC1001::Style->new($self->{_io}, $self, $self->{_root});
    }
    $self->{linetypes} = ();
    my $n_linetypes = $self->header()->table_linetype()->items();
    for (my $i = 0; $i < $n_linetypes; $i++) {
        push @{$self->{linetypes}}, CAD::Format::DWG::AC1001::Linetype->new($self->{_io}, $self, $self->{_root});
    }
    $self->{views} = ();
    my $n_views = $self->header()->table_view()->items();
    for (my $i = 0; $i < $n_views; $i++) {
        push @{$self->{views}}, CAD::Format::DWG::AC1001::View->new($self->{_io}, $self, $self->{_root});
    }
